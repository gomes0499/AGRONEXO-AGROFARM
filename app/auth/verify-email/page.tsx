...
export default async function VerifyEmailPage() {
  ...
  console.log("Verificação de email concluída. Metadados do usuário:", user.user_metadata);
  
  // Se tiver token de convite, processar aceitação de convite
  if (inviteToken) {
    console.log("Token de convite encontrado nos metadados:", inviteToken);
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error("Erro ao aceitar convite após verificação de email:", result.error);
        redirect(`/auth/login?verificationError=${encodeURIComponent("Erro ao processar convite: " + result.error)}`);
      }
    } catch (error) {
      console.error("Exceção ao processar convite após verificação de email:", error);
      
      // Se der erro, redirecionar para login
      redirect(`/auth/login?verificationError=${encodeURIComponent("Erro ao processar convite")}`);
    }
  }
