...
export async function getFluxoCaixaSimplificado(organizacaoId: string): Promise<FluxoCaixaData> {
  try {
    ...
    const { data: safras, error: safrasError } = await supabaseClient
      .from("safras")
      .select("*")
      .eq("organizacao_id", organizacaoId)
      .order("ano_inicio", { ascending: true });
    
    const safrasData = safras as Safra[] | null;
    ...
    await calcularDadosAgricolas(supabaseClient, organizacaoId, safrasData, fluxoCaixaData);
    
    // Obter dados de arrendamentos
    await calcularDadosArrendamento(supabaseClient, organizacaoId, safrasData, fluxoCaixaData);
    
    // Obter dados de outras despesas
    await calcularOutrasDespesas(supabaseClient, organizacaoId, safrasData, fluxoCaixaData);
    
    // Obter dados de investimentos
    await calcularDadosInvestimentos(supabaseClient, organizacaoId, safrasData, fluxoCaixaData);
    
    // Obter dados financeiros (dívidas bancárias)
    await calcularDadosFinanceiras(supabaseClient, organizacaoId, safrasData, fluxoCaixaData);
...
async function calcularDadosAgricolas(
  ...
  try {
    ...
    for (const cultura of culturas) {
      const culturaNome = cultura.nome as string;
      ...
      fluxoCaixaData.anos.forEach((ano: string) => {
      ...
      for (const safra of safras) {
        ...
        const areaSafra = areas?.find(a => a.safra_id === safra.id);
        const areaCultivada = areaSafra ? areaSafra.area : 0;
        
        // Encontrar produtividade para esta safra
        const produtividadeSafra = produtividades?.find(p => p.safra_id === safra.id);
        ...
        const precoSafra = precos?.sort((a, b) => 
...
async function calcularDadosInvestimentos(
  ...
  try {
    ...
    todosInvestimentos.forEach((investimento: any) => {
...
async function calcularDadosFinanceiras(
  ...
  try {
    ...
    safras.forEach((safra) => {
      ...
      // Serviço da dívida é 9.92% do total da dívida bancária
      // Valor fixo de 359.179.564 para todas as safras após 2023/24
      const servicoDivida = safra.ano_inicio >= 2023 ? 
        valorFixoDividasBancarias * 0.0992 : 0;
      
      // Pagamentos a bancos fixo em R$ 179M conforme tabela
...
function criarDadosDemoFluxoCaixa(): FluxoCaixaData {
  ...
  anos.forEach((ano: string, index: number) => {
    // Receitas agrícolas
    ...
    // Valor fixo para dívidas bancárias conforme tabela
    const valorFixoDividasBancarias = 359179564; // R$ 359.179.564
    
    // Serviço da dívida (9.92% do valor fixo da dívida bancária)
    // Só aplicar para anos >= 2023/24
    const servicoDivida = (ano === "2021/22" || ano === "2022/23") ? 
      0 : valorFixoDividasBancarias * 0.0992;
