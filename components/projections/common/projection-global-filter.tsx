...
interface ProjectionGlobalFilterProps {
  properties: Property[];
  cultures: Culture[];
  systems: System[];
  cycles: Cycle[];
...
function MultiSelectFilter<T>({
  ...
  const selectedCountText = selectedIds.length === 0 
    ? `Nenhum(a) ${label.toLowerCase()} selecionado(a)` 
    : selectedIds.length === items.length 
      ? `Todos(as) ${label.toLowerCase()}s` 
      : `${selectedIds.length} ${label.toLowerCase()}${selectedIds.length === 1 ? '' : 's'}`;
...
export function ProjectionGlobalFilter({
  properties,
  cultures,
  systems,
  cycles,
  ...
  onClearFilters,
}: ProjectionGlobalFilterProps) {
  const activeFiltersCount = [
    selectedPropertyIds.length > 0 && selectedPropertyIds.length < properties.length ? 1 : 0,
    selectedCultureIds.length > 0 && selectedCultureIds.length < cultures.length ? 1 : 0,
    selectedSystemIds.length > 0 && selectedSystemIds.length < systems.length ? 1 : 0,
    selectedCycleIds.length > 0 && selectedCycleIds.length < cycles.length ? 1 : 0,
    selectedSafraIds.length > 0 && selectedSafraIds.length < safras.length ? 1 : 0,
