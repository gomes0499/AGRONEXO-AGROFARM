...
import { AlertCircle, TrendingDown, ChartBar } from "lucide-react";
import { cn } from "@/lib/utils";

interface DebtPositionListingProps {
  organizationId: string;
  activeConfigId: string | null;
}

export function DebtPositionListing({
  organizationId,
  activeConfigId,
}: DebtPositionListingProps) {
    ...
    return (
      <Card className="shadow-sm border-muted/80">
        <CardHeaderPrimary
          ...
          description="Análise consolidada do endividamento e indicadores financeiros"
        />
          ...
          <EmptyState
            ...
            description={
              !activeConfigId
                ...
                : "Nenhum dado encontrado para os filtros selecionados. Verifique se há dados financeiros cadastrados."
            }
          />
        </CardContent>
      </Card>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-primary hover:bg-primary">
                  ...
                  <TableHead className="font-semibold text-primary-foreground min-w-[100px] w-[100px] sticky left-[200px] bg-primary z-20 border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                    Unidade
                  </TableHead>
                  {debtData.years.map((year, index) => (
                    <TableHead 
                      key={year.safraId} 
                      className={cn(
                        "font-semibold text-primary-foreground text-center min-w-[120px] w-[120px] whitespace-nowrap",
                        index === debtData.years.length - 1 && "rounded-tr-md"
                      )}
                    >
                      {year.safraName}
                    </TableHead>
