...
export function SeedSaleForm({
  ...
  const CurrencyField = ({
    ...
    control,
    placeholder = "R$ 0,00",
    isCost = false, // Adicionado para identificar campos de custo
    isRevenue = false, // Adicionado para identificar campos de receita
  }: {
    name: string;
    label: string;
    control: any;
    ...
    <FormField
      ...
      render={({ field }) => {
        ...
        return (
          <FormItem>
            <FormLabel>
              {isCost ? `(-) ${label}` : isRevenue ? `(+) ${label}` : label}
            </FormLabel>
            <FormControl>
              <input
