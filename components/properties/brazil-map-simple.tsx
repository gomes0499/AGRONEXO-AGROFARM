...
export function BrazilMapSimple({
  estadosData,
  className,
}: BrazilMapSimpleProps) {
  ...
  const getStateData = (geoProperties: any) => {
    ...
    if (
      countryName &&
      !["Brazil", "Brasil", "BRAZIL", "BRASIL"].includes(countryName)
    ) {
    ...
    for (const field of possibleFields) {
      ...
      for (const [code, name] of Object.entries(STATE_MAPPINGS)) {
        if (
          upperValue === name ||
          upperValue.includes(name) ||
          name.includes(upperValue)
        ) {
        ...
        <Geographies geography={BRAZIL_TOPO}>
          {({ geographies }) => {
            ...
            const brazilGeos = geographies.filter((geo) => {
              ...
              return (
                name && ["Brazil", "Brasil", "BRAZIL", "BRASIL"].includes(name)
              );
            });
