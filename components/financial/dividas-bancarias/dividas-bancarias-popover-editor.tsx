...
import { Loader2, Save, CalendarDays, Table } from "lucide-react";
import { toast } from "sonner";
import { getSafras } from "@/lib/actions/production-actions";
import { updateDividaBancaria } from "@/lib/actions/financial-actions/dividas-bancarias";
import { formatGenericCurrency, parseFormattedNumber } from "@/lib/utils/formatters";

interface DividasBancariasPopoverEditorProps {
  divida: any;
  organizationId: string;
  onUpdate: (updatedDivida: any) => void;
}

export function DividasBancariasPopoverEditor({
  divida,
  organizationId,
  onUpdate,
}: DividasBancariasPopoverEditorProps) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const updatedDivida = await updateDividaBancaria(
        divida.id,
        {
          nome: divida.nome,
        ...
        <div className="grid gap-4 w-[600px] max-h-[400px] overflow-y-auto">
          <div className="flex items-center gap-2 mb-2">
            ...
            <Badge variant="outline" className="ml-auto">
              {divida.nome} • {divida.modalidade || "Sem modalidade"} • {divida.moeda || "BRL"}
            </Badge>
          </div>
