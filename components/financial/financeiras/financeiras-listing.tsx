...
import { PlusIcon, ReceiptIcon, ChevronDown, ChevronUp } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
interface FinanceirasListingProps {
  ...
  initialItems: FinanceirasListItem[];
}
...
export function FinanceirasListing({
  ...
  // Adicionar novo item
  const handleAddItem = (newItem: FinanceirasListItem) => {
  ...
  const handleUpdateItem = (updatedItem: FinanceirasListItem) => {
    setItems(
      items.map((item) =>
  ...
  const calculateTotal = (item: FinanceirasListItem) => {
    let total = 0;
  ...
  const formatCategoria = (categoria: FinanceirasCategoriaType) => {
        ...
        <div className="space-y-4">
          ...
          {items.length === 0 ? (
            <div className="text-center py-10 text-muted-foreground space-y-4">
              <div>Nenhuma operação financeira cadastrada.</div>
              <Button 
                onClick={() => setIsAddModalOpen(true)}
