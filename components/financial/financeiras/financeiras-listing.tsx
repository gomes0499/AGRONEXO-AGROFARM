...
import { FinanceirasListItem, FinanceirasCategoriaType } from "@/schemas/financial/financeiras";
import { Button } from "@/components/ui/button";
import { PlusIcon, ReceiptIcon, ChevronDown, ChevronUp, FileSpreadsheet, Loader2 } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
import { deleteFinanceiras, getFinanceiras } from "@/lib/actions/financial-actions/financeiras";
...
import { FinanceirasSafraDetail } from "./financeiras-safra-detail";
import { FinanceirasImportDialog } from "./financeiras-import-dialog";
import { EmptyState } from "@/components/shared/empty-state";

interface FinanceirasListingProps {
  organization: { id: string; nome: string };
  initialFinanceiras: FinanceirasListItem[];
  projectionId?: string;
  error?: string;
  safras?: any[];
}

export function FinanceirasListing({
  organization,
  initialFinanceiras,
  projectionId,
  error: initialError,
  safras = [],
}: FinanceirasListingProps) {
  const [items, setItems] = useState<
    (FinanceirasListItem & { isExpanded?: boolean })[]
  >((initialFinanceiras || []).map(item => ({ ...item, isExpanded: false })));
  const [error, setError] = useState<string | null>(initialError || null);
  const [isPending, startTransition] = useTransition();
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isImportModalOpen, setIsImportModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<FinanceirasListItem | null>(null);
  ...
  const refreshData = useCallback(() => {
    startTransition(async () => {
      try {
        const newItems = await getFinanceiras(organization.id);
        setItems(newItems.map(item => ({ ...item, isExpanded: false })));
        setError(null);
      } catch (err) {
        console.error("❌ Erro ao atualizar financeiras:", err);
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erro desconhecido ao carregar dados";
        setError(`Erro ao buscar operações financeiras: ${errorMessage}`);
      }
    });
  ...
  // Adicionar novo item
  const handleAddItem = useCallback((newItem: FinanceirasListItem) => {
    setItems(prev => [{ ...newItem, isExpanded: false }, ...prev]);
    setIsAddModalOpen(false);
    toast.success("Item financeiro adicionado com sucesso.");
  }, []);

  // Importar itens via Excel
  const handleImportSuccess = useCallback((importedItems: FinanceirasListItem[]) => {
    const itemsWithExpanded = importedItems.map(item => ({ ...item, isExpanded: false }));
    setItems(prev => [...itemsWithExpanded, ...prev]);
  ...
  const handleUpdateItem = useCallback((updatedItem: FinanceirasListItem) => {
    setItems(prev =>
      prev.map((item) =>
  ...
  const calculateTotal = useCallback((item: FinanceirasListItem) => {
    let total = 0;
  ...
  const formatCategoria = useCallback((categoria: FinanceirasCategoriaType) => {
    const formatMap: Record<FinanceirasCategoriaType, string> = {
  ...
  if (error) {
    return (
      <EmptyState
        icon={<ReceiptIcon className="h-10 w-10 text-destructive" />}
        title="Erro ao carregar operações financeiras"
        description={error}
        action={
    ...
    <div className="space-y-6 relative">
      ...
      <Card className="shadow-sm border-muted/80">
        <CardHeaderPrimary
          ...
          description="Gestão de operações financeiras e linhas de crédito"
          action={
          ...
          <div className="space-y-4">
            ...
            {items.length === 0 ? (
              <EmptyState
                ...
                title="Nenhuma operação financeira cadastrada"
                description="Comece adicionando suas operações financeiras e linhas de crédito."
                action={
