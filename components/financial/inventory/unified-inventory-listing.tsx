...
import { Loader2, Pencil, Save, Package, PlusCircle } from "lucide-react";
...
type UnifiedInventoryListingProps = {
  inventories: InventoryType[] | undefined;
  safras: Array<{ id: string; nome: string; ano_inicio: number }>;
};

export function UnifiedInventoryListing({ 
  inventories = [],
  safras = []
}: UnifiedInventoryListingProps) {
  
  // Se não há estoques, mostrar estado vazio
      ...
      <Card className="w-full shadow-sm border-muted/80">
        <CardHeader className="bg-primary text-white rounded-t-lg flex flex-row items-center justify-between space-y-0 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-white">Estoques</CardTitle>
              <CardDescription className="text-white/80">
                Gerencie estoques de fertilizantes, defensivos e almoxarifado por safra
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="mt-4">
          <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-4">
            <p className="text-amber-700 mb-4">
              Não encontramos estoques configurados. Se você acabou de criar as tabelas, tente recarregar a página para inicializar os valores padrão.
            </p>
            <Button 
  ...
  const initEditState = (inventory: InventoryType) => {
    if (!editingState[inventory.id]) {
      const editValues: Record<string, string> = {};
      
      yearsToShow.forEach(year => {
        const safra = safras.find(s => s.ano_inicio === year);
        if (safra && inventory.valores_por_ano[safra.id]) {
          editValues[`value${year}`] = inventory.valores_por_ano[safra.id].toString();
        } else {
      ...
      setEditingState(prev => ({
        ...prev,
        [inventory.id]: editValues,
      }));
    }

    if (isLoading[inventory.id] === undefined) {
      setIsLoading(prev => ({
        ...prev,
        [inventory.id]: false,
      }));
  ...
  // Salvar mudanças
  const handleSave = async (inventory: InventoryType) => {
    try {
      setIsLoading(prev => ({
        ...prev,
        [inventory.id]: true,
      ...
      const editValues = editingState[inventory.id];
      ...
      inventory.valores_por_ano = newValoresPorAno;
  ...
  const getValueForYear = (inventory: InventoryType, year: number): number => {
    const safra = safras.find(s => s.ano_inicio === year);
    if (safra && inventory.valores_por_ano[safra.id]) {
      return inventory.valores_por_ano[safra.id];
    }
    return 0;
  };
          ...
          {inventories.map((inventory) => {
            ...
            return (
              <TableRow key={inventory.id}>
                ...
                <TableCell>
                  {getTypeBadge(inventory.tipo)}
                </TableCell>
                {yearsToShow.map(year => (
                  <TableCell key={year}>
                    {formatCurrencyCompact(getValueForYear(inventory, year))}
                  </TableCell>
                ))}
                <TableCell>
                  <Popover>
                      ...
                      <div className="grid gap-4 w-[800px] max-h-[500px] overflow-y-auto">
                        <div className="space-y-2">
                          <h4 className="font-medium leading-none">
                            Editar Valores - {inventory.descricao}
                          </h4>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {yearsToShow.map((year) => (
                            <div key={year} className="space-y-2">
                              <Label htmlFor={`${inventory.id}-value${year}`}>
                                {year}/{String(year + 1).slice(-2)}
                              </Label>
                              <Input
                                id={`${inventory.id}-value${year}`}
                                type="number"
                                value={
                                  editingState[inventory.id]?.[`value${year}`] ||
                                  getValueForYear(inventory, year).toString()
                                }
                                onChange={(e) =>
                                  handleInputChange(
                                    inventory.id,
                        ...
                        <Button
                          ...
                          disabled={isLoading[inventory.id]}
                          className="w-full"
                        >
                          {isLoading[inventory.id] ? (
                            <>
