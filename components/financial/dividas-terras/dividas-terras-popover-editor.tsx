...
import { updateDividaTerra } from "@/lib/actions/financial-actions/dividas-terras";
import { formatGenericCurrency, parseFormattedNumber } from "@/lib/utils/formatters";

interface DividasTerrasPopoverEditorProps {
  divida: any;
  organizationId: string;
  onUpdate: (updatedDivida: any) => void;
}

export function DividasTerrasPopoverEditor({
  divida,
  organizationId,
  onUpdate,
}: DividasTerrasPopoverEditorProps) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const updatedDivida = await updateDividaTerra(
        divida.id,
        {
          nome: divida.nome,
  ...
  const getPropertyName = () => {
    return divida.propriedade_nome || divida.propriedades?.nome || "Sem propriedade";
  };
