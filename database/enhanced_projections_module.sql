-- ==========================================
-- Módulo de Projeções Aprimorado
-- Migração para criar estrutura completa de projeções
-- ==========================================

-- Adicionar novos tipos enumerados para projeções
...
-- ==========================================
-- Aprimorar tabela de projeções de culturas
-- ==========================================

-- Renomear e aprimorar a tabela existente
ALTER TABLE public.projecoes_culturas RENAME TO projecoes_culturas_old;
...
-- ==========================================
-- Aprimorar tabela de projeções de dívidas
-- ==========================================

-- Renomear tabela existente
ALTER TABLE public.projecoes_dividas RENAME TO projecoes_dividas_old;
...
-- ==========================================
-- Aprimorar tabela de projeções de caixa
-- ==========================================

-- Renomear tabela existente
ALTER TABLE public.projecoes_caixa_disponibilidades RENAME TO projecoes_caixa_old;
...
-- ==========================================
-- Aprimorar tabela de fluxo de caixa
-- ==========================================

-- Renomear tabela existente
ALTER TABLE public.projecoes_fluxo_caixa RENAME TO projecoes_fluxo_caixa_old;
...
-- ==========================================
-- Nova tabela para cenários de projeção
-- ==========================================
...
-- ==========================================
-- Nova tabela para relatórios consolidados
-- ==========================================
...
-- Criar configuração padrão para organizações que já têm projeções
INSERT INTO public.projecoes_config (organizacao_id, nome, periodo_inicio, periodo_fim)
SELECT DISTINCT 
  organizacao_id, 
  'Projeção Padrão',
  EXTRACT(YEAR FROM MIN(created_at))::INT,
  EXTRACT(YEAR FROM MIN(created_at))::INT + 10
FROM public.projecoes_culturas_old
GROUP BY organizacao_id;

-- Migrar dados de projeções de culturas
INSERT INTO public.projecoes_culturas (
  organizacao_id, projecao_config_id, cultura_id, sistema_id, periodo,
  area_plantada, produtividade, unidade_produtividade, preco_unitario
)
SELECT 
  old.organizacao_id,
  config.id,
  old.cultura_id,
  old.sistema_id,
  COALESCE(s.nome, EXTRACT(YEAR FROM old.created_at)::TEXT),
  old.area,
  old.produtividade,
  old.unidade,
  old.preco
FROM public.projecoes_culturas_old old
JOIN public.projecoes_config config ON config.organizacao_id = old.organizacao_id 
  AND config.nome = 'Projeção Padrão'
LEFT JOIN public.safras s ON s.id = old.safra_id;
...
-- Índices para projecoes_culturas aprimoradas
...
-- Índices para projecoes_dividas aprimoradas
...
-- Índices para projecoes_caixa_disponibilidades aprimoradas
...
-- Índices para projecoes_fluxo_caixa aprimoradas
...
CREATE TRIGGER update_timestamp_projecoes_culturas_new
BEFORE UPDATE ON public.projecoes_culturas
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_timestamp_projecoes_dividas_new
BEFORE UPDATE ON public.projecoes_dividas
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_timestamp_projecoes_caixa_new
BEFORE UPDATE ON public.projecoes_caixa_disponibilidades
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_timestamp_projecoes_fluxo_new
...
CREATE TRIGGER audit_changes_projecoes_culturas_new
AFTER INSERT OR UPDATE OR DELETE ON public.projecoes_culturas
FOR EACH ROW EXECUTE FUNCTION public.audit_changes();

CREATE TRIGGER audit_changes_projecoes_dividas_new
AFTER INSERT OR UPDATE OR DELETE ON public.projecoes_dividas
FOR EACH ROW EXECUTE FUNCTION public.audit_changes();

CREATE TRIGGER audit_changes_projecoes_caixa_new
AFTER INSERT OR UPDATE OR DELETE ON public.projecoes_caixa_disponibilidades
FOR EACH ROW EXECUTE FUNCTION public.audit_changes();

CREATE TRIGGER audit_changes_projecoes_fluxo_new
...
-- ==========================================
-- Função para validar projeção padrão única
-- ==========================================
...
-- ==========================================
-- Função para validar cenário base único
-- ==========================================
...
-- ==========================================
-- Cleanup opcional das tabelas antigas
-- (Descomentar após verificar migração)
-- ==========================================

-- DROP TABLE IF EXISTS public.projecoes_culturas_old;
-- DROP TABLE IF EXISTS public.projecoes_dividas_old;
-- DROP TABLE IF EXISTS public.projecoes_caixa_old;
-- DROP TABLE IF EXISTS public.projecoes_fluxo_caixa_old;