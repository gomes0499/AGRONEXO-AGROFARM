-- Update create_projection function to include investimentos and financeiras tables
...
BEGIN
  ...
  -- Copy investimentos
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'investimentos_projections') THEN
    INSERT INTO investimentos_projections (
      projection_id, organizacao_id, safra_id, categoria, ano,
      quantidade, valor_unitario, valor_total, tipo,
      created_at, updated_at, original_investimento_id
    )
    SELECT 
      v_projection_id, organizacao_id, safra_id, categoria, ano,
      quantidade, valor_unitario, valor_total, tipo,
      created_at, CURRENT_TIMESTAMP, id
    FROM investimentos
    WHERE organizacao_id = p_organizacao_id;
  ...
  -- Copy financeiras
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'financeiras_projections') THEN
    INSERT INTO financeiras_projections (
      projection_id, organizacao_id, nome, categoria, valores_por_ano,
      created_at, updated_at, original_financeira_id
    )
    SELECT 
      v_projection_id, organizacao_id, nome, categoria, valores_por_ano,
      created_at, CURRENT_TIMESTAMP, id
    FROM financeiras
    WHERE organizacao_id = p_organizacao_id;
  END IF;
