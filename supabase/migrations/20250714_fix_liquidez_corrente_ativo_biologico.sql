-- Fix liquidez corrente calculation to include ATIVO_BIOLOGICO
-- Date: 2025-07-14
-- Description: Update calculate_rating_metrics_optimized to include ATIVO_BIOLOGICO in current assets
...
BEGIN
  ...
  -- Get land debt specifically for LTV (only current year land debt)
  SELECT COALESCE(SUM(
    CASE 
      WHEN jsonb_typeof(di.fluxo_pagamento_anual) = 'object'
...
VALUES ('Fixed liquidez corrente calculation to include ATIVO_BIOLOGICO in calculate_rating_metrics_optimized', NOW());